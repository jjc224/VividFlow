...
int                isSupportedImageFile(const char *filename);    // TODO: handle imported images in some generic way. This function will no longer be necessary.
...
struct ModuleInterface *initInterface(int argc, char *argv[])
{
	moduleInterface = allocateMemory(sizeof(struct ModuleInterface));

	moduleInterface->numOfArgs   = (argc + 1) / 4;    // TODO: handle this better (this allocates an extra in some cases).
	moduleInterface->ioArguments = allocateMemory(sizeof(struct IOInterface) * moduleInterface->numOfArgs);
...
void writeGenericOutput(struct IOInterface *args)    // Used to accept a outputType; no longer appears necessary.
{
//	if(type == OT_IMAGEFILE)
//	{
//		if(isSupportedImageFile(args->inputFile) != 0)
//			fprintf(stderr, "writeGenericOutput(): unsupported image type for input file '%s'.\n", args->inputFile);
//
//		if(isSupportedImageFile(args->outputFile) != 0)
//			fprintf(stderr, "writeGenericOutput(): unsupported image type for output file '%s'.\n", args->outputFile);
//
//		// Error handling within the chain?
//	}
...
struct GenericData *readGenericInput(struct IOInterface *args)
{
	...
	struct GenericData *fileData = allocateMemory(sizeof(struct GenericData));
...
int isSupportedImageFile(const char *filename)
{
	...
	const char *supportedExtensions      = {"vfimg"};
	const size_t supportedExtensionsSize = sizeof(supportedExtensions) / sizeof(supportedExtensions[0]);

	for(int i = 0; i < supportedExtensionsSize; ++i)
	{
		if(strcmp(filenameExtension, &supportedExtensions[i]) == 0)
			return 0;
	}
...
struct OutputData *getDataFromTextFile(const char *filename, enum outputType type)
{
	struct OutputData *data = allocateMemory(sizeof(struct OutputData));
	FILE *file              = openFile(filename, "r");

	switch(type)
	{
		case OT_STRING:
			data->options.string = allocateMemory(sizeof(char) * MAX_STRING_SIZE);
			fgets(data->options.string, MAX_STRING_SIZE, file);      // MAX_STRING_SIZE = 256.
...
void writeString(int argNum, const char *string)
{
	...
	data.type           = OT_STRING;
	data.options.string = allocateMemory(strlen(string + 1));
